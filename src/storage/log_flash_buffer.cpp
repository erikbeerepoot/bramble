...
{
    if (page_buffer_count_ == 0) return true;
...
bool LogFlashBuffer::ensureSectorErased(uint32_t address)
{
    ...
    if (result == ExternalFlashResult::Success) {
        bool erased = (check[0] == 0xFF && check[1] == 0xFF &&
...
{
    if (!initialized_ || index >= MAX_RECORDS) return false;

    uint32_t address = getRecordAddress(index);
    ExternalFlashResult result = flash_.read(
        address, reinterpret_cast<uint8_t*>(&record), sizeof(record));
    if (result != ExternalFlashResult::Success) return false;

    // Verify CRC
    uint16_t calculated = CRC16::calculate(
        reinterpret_cast<const uint8_t*>(&record),
        sizeof(LogRecord) - sizeof(record.crc16));
...
uint32_t LogFlashBuffer::getStoredCount() const
{
    ...
    if (metadata_.total_records >= MAX_RECORDS) return MAX_RECORDS;
...
bool LogFlashBuffer::flush()
{
    ...
    if (page_buffer_count_ > 0) {
        if (!flushPageBuffer()) return false;
    }
    return saveMetadata();
}
...
{
    ExternalFlashResult result = flash_.read(
        METADATA_OFFSET, reinterpret_cast<uint8_t*>(&metadata_), sizeof(metadata_));
    if (result != ExternalFlashResult::Success) return false;
...
bool LogFlashBuffer::saveMetadata()
{
    metadata_.crc32 = ConfigurationBase::calculateCRC32(
        ...
        sizeof(metadata_) - sizeof(metadata_.crc32));

    ExternalFlashResult result = flash_.eraseSector(METADATA_OFFSET);
    if (result != ExternalFlashResult::Success) return false;

    result = flash_.write(
        METADATA_OFFSET, reinterpret_cast<const uint8_t*>(&metadata_), sizeof(metadata_));
...
{
    if (metadata_.magic != LOG_FLASH_MAGIC) return false;
    if (metadata_.version != LOG_FLASH_VERSION) return false;

    uint32_t calculated = ConfigurationBase::calculateCRC32(
        reinterpret_cast<const uint8_t*>(&metadata_),
        sizeof(metadata_) - sizeof(metadata_.crc32));
    if (calculated != metadata_.crc32) return false;

    if (metadata_.write_index >= MAX_RECORDS) return false;
    return true;
}
