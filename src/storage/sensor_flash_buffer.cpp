...
bool SensorFlashBuffer::markTransmitted(uint32_t index) {
    ...
    ExternalFlashResult result = flash_.read(address,
...
bool SensorFlashBuffer::readUntransmittedRecords(SensorDataRecord* records,
                                                  size_t max_count,
                                                  size_t& actual_count) {
    if (!initialized_) {
        logger_.error("Buffer not initialized");
...
bool SensorFlashBuffer::loadMetadata() {
    ExternalFlashResult result = flash_.read(METADATA_SECTOR,
                                             reinterpret_cast<uint8_t*>(&metadata_),
                                             sizeof(metadata_));
    if (result != ExternalFlashResult::Success) {
...
bool SensorFlashBuffer::saveMetadata() {
    ...
    // Write metadata
    result = flash_.write(METADATA_SECTOR,
                         reinterpret_cast<const uint8_t*>(&metadata_),
                         sizeof(metadata_));
    if (result != ExternalFlashResult::Success) {
...
bool SensorFlashBuffer::validateMetadata() const {
    ...
    if (calculated_crc != metadata_.crc32) {
        logger_.warn("CRC mismatch: expected 0x%08X, got 0x%08X",
                     metadata_.crc32, calculated_crc);
        return false;
    }

    // Validate indices
    if (metadata_.write_index >= MAX_RECORDS || metadata_.read_index >= MAX_RECORDS) {
        logger_.warn("Invalid indices: write=%lu, read=%lu",
                     metadata_.write_index, metadata_.read_index);
        return false;
    }
