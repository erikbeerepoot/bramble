...
void ApplicationMode::handleSetDatetime(const UpdateAvailablePayload *payload, uint8_t hub_sequence)
{
    if (!reliable_pmu_) {
    ...
    reliable_pmu_->setDateTime(datetime, [this, hub_sequence](bool success, PMU::ErrorCode error) {
        if (success) {
