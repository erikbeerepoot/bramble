...
public:
    ...
    void update(uint32_t current_time) override
    {
        ...
        if (current_time - last_update_ < 15)
            return;
...
public:
    BlinkingPattern(NeoPixel &led, uint8_t r, uint8_t g, uint8_t b, uint32_t on_ms = 250,
                    uint32_t off_ms = 250)
...
public:
    ShortBlinkPattern(NeoPixel &led, uint8_t r, uint8_t g, uint8_t b, uint32_t on_ms = 50,
                      uint32_t off_ms = 500)
        : LEDPattern(led), r_(r), g_(g), b_(b), on_ms_(on_ms), off_ms_(off_ms)
    {
    }
