...
bool ReliablePmuClient::init() {
    ...
    client_->getProtocol().setAckCallback(
        [this](uint8_t seqNum, bool success, ErrorCode error) {
    ...
    client_->getProtocol().onScheduleComplete(
        [this]() {
    ...
    client_->getProtocol().onWakeInterval(
        [this](uint32_t seconds) {
            if (wakeIntervalCallback_) {
                wakeIntervalCallback_(seconds);
            }
        });

    client_->getProtocol().onScheduleEntry(
        [this](const ScheduleEntry& entry) {
