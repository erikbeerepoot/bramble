...
SPIDevice::SPIDevice(spi_inst_t *spi_port, uint cs_pin)
    : spi_(spi_port), cs_pin_(cs_pin), read_mask_(0x7F), write_mask_(0x80), logger_("SPIDevice")
{
    // Initialize CS pin
    gpio_init(cs_pin_);
...
SPIError SPIDevice::writeRegister(uint8_t reg, uint8_t value, bool verify)
{
    ...
    if (verify) {
        ...
        if (read_value != value) {
            logger_.warn("Write verify failed reg 0x%02X: wrote 0x%02X, read 0x%02X", reg, value,
                         read_value);
            return SPI_ERROR_TRANSFER;
        }
    }
...
SPIError SPIDevice::readBuffer(uint8_t reg, uint8_t *data, size_t length, uint8_t *tx_buf,
                               uint8_t *rx_buf)
{
