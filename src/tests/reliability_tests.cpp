...
{
    // Create ACK message
    MessageHeader ack_header = {.magic = MESSAGE_MAGIC,
                                .type = MSG_TYPE_ACK,
                                .flags = 0,
                                .src_addr = src_addr,
                                .dst_addr = node_addr_,
    ...
    AckPayload ack_payload = {.ack_seq_num = seq_num, .status = 0};
...
bool ReliabilityTestHarness::sendBestEffortMessage(uint16_t dst_addr, const uint8_t *data,
                                                   size_t length)
{
    uint8_t buffer[MESSAGE_MAX_SIZE];
    MessageHandler handler;

    size_t msg_len = handler.createSensorMessage(node_addr_, dst_addr, next_seq_num_++,
                                                 SENSOR_TEMPERATURE, data, length, 0, buffer);

    return sendMessage(buffer, msg_len, false, false);
}

bool ReliabilityTestHarness::sendReliableMessage(uint16_t dst_addr, const uint8_t *data,
                                                 size_t length)
{
    uint8_t buffer[MESSAGE_MAX_SIZE];
    MessageHandler handler;
    ...
    size_t msg_len =
        handler.createSensorMessage(node_addr_, dst_addr, next_seq_num_, SENSOR_SOIL_MOISTURE, data,
                                    length, MSG_FLAG_RELIABLE, buffer);

    return sendMessage(buffer, msg_len, true, false);
}

bool ReliabilityTestHarness::sendCriticalMessage(uint16_t dst_addr, const uint8_t *data,
                                                 size_t length)
{
    uint8_t buffer[MESSAGE_MAX_SIZE];
    MessageHandler handler;

    size_t msg_len = handler.createActuatorMessage(node_addr_, dst_addr, next_seq_num_,
...
bool ReliabilityTestHarness::sendMessage(const uint8_t *buffer, size_t length, bool reliable,
                                         bool critical)
{
