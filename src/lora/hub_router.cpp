...
HubRouter::HubRouter(AddressManager &address_manager, ReliableMessenger &messenger)
    : address_manager_(address_manager), messenger_(messenger), total_messages_routed_(0),
      total_messages_queued_(0), total_messages_dropped_(0)
{
}
...
bool HubRouter::processMessage(const uint8_t *buffer, size_t length, uint16_t source_address)
{
    ...
    if (header->dst_addr != ADDRESS_HUB && header->dst_addr != ADDRESS_BROADCAST) {
        // This is a node-to-node message that needs routing
        logger.info("Routing message from 0x%04X to 0x%04X", source_address, header->dst_addr);
...
bool HubRouter::forwardMessage(const uint8_t *buffer, size_t length, uint16_t destination_address,
...
void HubRouter::updateRouteOnline(uint16_t node_address)
{
    ...
    bool was_offline =
        !route.is_online || (current_time - route.last_online_time > NODE_OFFLINE_TIMEOUT_MS);
...
void HubRouter::getRoutingStats(uint32_t &total_routed, uint32_t &total_queued,
...
void HubRouter::printRoutingTable()
{
    ...
    for (const auto &entry : routing_table_) {
        ...
        logger.info("0x%04X      | %s   | %lu", route.destination_address,
                    route.is_online ? "Yes" : "No ", route.last_used_time);
    }

    logger.info("=====================");
    logger.info("Messages routed: %lu, queued: %lu, dropped: %lu", total_messages_routed_,
                total_messages_queued_, total_messages_dropped_);
}
...
uint32_t HubRouter::clearOldRoutes(uint32_t current_time, uint32_t max_age_ms)
{
    ...
    while (it != routing_table_.end()) {
        ...
        bool should_mark_offline =
            (current_time - it->second.last_online_time > NODE_OFFLINE_TIMEOUT_MS);

        if (should_mark_offline && it->second.is_online) {
            logger.info("Node 0x%04X went offline (inactive for %d minutes)", it->first,
                        NODE_OFFLINE_TIMEOUT_MS / (60 * 1000));
...
bool HubRouter::queueMessage(const uint8_t *buffer, size_t length, uint16_t destination_address,
                             bool requires_ack)
{
    ...
    logger.debug("Queued message for 0x%04X (queue size: %zu)", destination_address,
                 message_queue_.size());
                                    ...
                                    const PMU::ScheduleEntry &entry)
{
    ...
    logger.debug("Queued schedule update for node 0x%04X (seq=%d, pos=%zu)", node_addr,
                 update.sequence, state.pending_updates.size());
...
bool HubRouter::queueRemoveSchedule(uint16_t node_addr, uint8_t index)
{
    ...
    logger.debug("Queued schedule removal for node 0x%04X (seq=%d, index=%d)", node_addr,
...
bool HubRouter::queueDateTimeUpdate(uint16_t node_addr, const PMU::DateTime &datetime)
{
    ...
    logger.debug("Queued datetime update for node 0x%04X (seq=%d)", node_addr, update.sequence);
...
bool HubRouter::queueWakeIntervalUpdate(uint16_t node_addr, uint16_t interval_seconds)
{
    ...
    logger.debug("Queued wake interval update for node 0x%04X (seq=%d, interval=%d)", node_addr,
                 update.sequence, interval_seconds);
...
void HubRouter::handleCheckUpdates(uint16_t node_addr, uint8_t node_sequence)
{
    ...
    while (!state.pending_updates.empty()) {
        ...
        if (front.sequence <= node_sequence) {
            logger.debug("Node 0x%04X: Already processed seq=%d, removing from queue", node_addr,
                         front.sequence);
    ...
    logger.debug("Node 0x%04X: Sending update seq=%d type=%d (node at seq=%d)", node_addr,
                 update.sequence, static_cast<int>(update.type), node_sequence);
...
void HubRouter::handleUpdateAck(uint16_t node_addr, uint8_t sequence, bool success,
                                uint8_t error_code)
{
    ...
    if (update.sequence != sequence) {
        logger.warn("Node 0x%04X: ACK sequence mismatch (expected %d, got %d)", node_addr,
                    update.sequence, sequence);
        return;
    }

    if (success) {
        logger.info("Node 0x%04X: Update seq=%d applied successfully", node_addr, sequence);

        // Remove from queue
        state.pending_updates.pop();
    } else {
        logger.error("Node 0x%04X: Update seq=%d failed (error=%d)", node_addr, sequence,
                     error_code);
...
void HubRouter::printQueueStats() const
{
    ...
    for (const auto &entry : node_updates_) {
        if (!entry.second.pending_updates.empty()) {
            logger.info("  Node 0x%04X: %zu updates (next_seq=%d)", entry.first,
                        entry.second.pending_updates.size(), entry.second.next_sequence);
        }
    }
