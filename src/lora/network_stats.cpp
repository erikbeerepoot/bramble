...
void NetworkStats::printSummary() const
{
    ...
    logger_.info("Nodes: %u registered, %u active, %u inactive", global_stats_.nodes_registered,
                 global_stats_.nodes_active, global_stats_.nodes_inactive);
    ...
    logger_.info("  Best Effort: %.1f%% delivery",
                 global_stats_.criticality_totals[BEST_EFFORT].getDeliveryRate());
...
void NetworkStats::printNodeStats(uint16_t address) const
{
    ...
    logger_.info("    Best Effort: %lu sent, %.1f%% delivered",
                 node->criticality_stats[BEST_EFFORT].sent,
                 node->criticality_stats[BEST_EFFORT].getDeliveryRate());
    logger_.info("    Reliable: %lu sent, %.1f%% delivered, %.2f avg retries",
                 node->criticality_stats[RELIABLE].sent,
                 node->criticality_stats[RELIABLE].getDeliveryRate(),
                 node->criticality_stats[RELIABLE].getAverageRetries());
    logger_.info("    Critical: %lu sent, %.1f%% delivered, %.2f avg retries, max %u",
                 node->criticality_stats[CRITICAL].sent,
                 node->criticality_stats[CRITICAL].getDeliveryRate(),
                 node->criticality_stats[CRITICAL].getAverageRetries(),
                 node->criticality_stats[CRITICAL].max_retries);
    ...
    if (node->rssi_stats.getSampleCount() > 0) {
        ...
        logger_.info("  Mean: %.1f dBm (σ=%.1f)", node->rssi_stats.getMean(),
                     node->rssi_stats.getStdDev());
        ...
        logger_.info("  Percentiles - 10th: %d, 50th: %d, 90th: %d dBm",
                     node->rssi_stats.getPercentile(10), node->rssi_stats.getPercentile(50),
                     node->rssi_stats.getPercentile(90));
        logger_.info("  Trend: %s",
                     node->rssi_stats.isTrendingUp() ? "Improving" : "Stable/Declining");
    }
    ...
    if (node->snr_stats.getSampleCount() > 0) {
        ...
        logger_.info("  Range: %.1f to %.1f dB", node->snr_stats.getMin() / 10.0f,
                     node->snr_stats.getMax() / 10.0f);
    }
