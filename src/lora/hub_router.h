 ...
 *
 * Currently supports direct routing only. Multi-hop fields (next_hop_address,
 * hop_count, is_direct) can be added when multi-hop routing is implemented.
 */
struct RouteEntry {
...
public:
     ...
     */
    bool forwardMessage(const uint8_t *buffer, size_t length, uint16_t destination_address,
     ...
     */
    uint32_t clearOldRoutes(uint32_t current_time,
                            uint32_t max_age_ms = 3600000);  // 1 hour default
...
private:
     ...
     * @return true if queued successfully
     */
    bool queueMessage(const uint8_t *buffer, size_t length, uint16_t destination_address,
                      bool requires_ack);
