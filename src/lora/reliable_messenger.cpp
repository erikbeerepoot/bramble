...
bool ReliableMessenger::sendActuatorCommand(uint16_t dst_addr, uint8_t actuator_type,
                                            uint8_t command, const uint8_t *params,
                                            uint8_t param_length, DeliveryCriticality criticality)
{
    ...
    return sendWithBuilder(
        [=](uint8_t *buffer) {
            size_t msg_size =
                MessageBuilder::createActuatorMessage(node_addr_, dst_addr, seq_num, actuator_type,
            ...
            if (msg_size > 0) {
                ...
                logger_.info(
                    "  Header: magic=0x%04X type=%d flags=0x%02X src=0x%04X dst=0x%04X seq=%d",
                    msg->header.magic, msg->header.type, msg->header.flags, msg->header.src_addr,
                    msg->header.dst_addr, msg->header.seq_num);
                ...
                const ActuatorPayload *actuator =
                    reinterpret_cast<const ActuatorPayload *>(msg->payload);
                ...
                size_t expected =
                    MESSAGE_HEADER_SIZE + 3 + param_length;  // header + actuator header + params
                                       ...
                                       uint8_t data_length, DeliveryCriticality criticality)
{
    ...
    logger_.debug("sendSensorData: src=0x%04X, dst=0x%04X, type=%d, criticality=%d", node_addr_,
                  dst_addr, sensor_type, criticality);

    return sendWithBuilder(
        [=](uint8_t *buffer) {
            return MessageBuilder::createSensorMessage(node_addr_, dst_addr, seq_num, sensor_type,
                                                 ...
                                                 uint8_t retry_interval, uint32_t network_time)
{
    ...
    return sendWithBuilder(
        [=](uint8_t *buffer) {
            return MessageHandler::createRegistrationResponse(node_addr_, dst_addr, seq_num,
                                                              device_id, assigned_addr, status,
                                                              retry_interval, network_time, buffer);
        },
...
bool ReliableMessenger::send(const uint8_t *buffer, size_t length, DeliveryCriticality criticality)
{
    if (!lora_ || !buffer || length == 0)
        return false;
                                            ...
                                            AckCallback ack_callback, uint64_t user_context)
{
    ...
    if (criticality == BEST_EFFORT) {
        logger_.debug(
            "Sending BEST_EFFORT with callback - note: callbacks only work for RELIABLE/CRITICAL");
...
uint8_t ReliableMessenger::sendSensorDataBatchWithCallback(
    uint16_t dst_addr, uint32_t start_index, const BatchSensorRecord *records, uint8_t record_count,
    DeliveryCriticality criticality, AckCallback ack_callback, uint64_t user_context)
{
    ...
    logger_.info("sendSensorDataBatch: src=0x%04X, dst=0x%04X, start_idx=%lu, count=%d", node_addr_,
                 dst_addr, start_index, record_count);

    uint8_t buffer[MESSAGE_MAX_SIZE];
    size_t length = MessageBuilder::createSensorDataBatchMessage(
        node_addr_, dst_addr, seq_num, start_index, records, record_count, flags, buffer);
...
bool ReliableMessenger::processIncomingMessage(const uint8_t *buffer, size_t length)
{
    ...
    if (message->header.dst_addr != node_addr_ && message->header.dst_addr != ADDRESS_BROADCAST) {
        // Message not for us - ignore it
        logger_.debug("Ignoring message for 0x%04X (our address: 0x%04X)", message->header.dst_addr,
                      node_addr_);
        return false;
    }
    ...
    if (message->header.type == MSG_TYPE_BATCH_ACK) {
        const BatchAckPayload *batch_ack =
            reinterpret_cast<const BatchAckPayload *>(message->payload);
        ...
        handleAck(&ack);
        logger_.info("Handled BATCH_ACK: seq=%d, status=%d, records=%d", batch_ack->ack_seq_num,
                     batch_ack->status, batch_ack->records_received);
        return true;
    }
    ...
    if (message->header.flags & MSG_FLAG_RELIABLE) {
        ...
        logger_.info("Received %s message requiring ACK", is_critical ? "CRITICAL" : "RELIABLE");
    ...
    if (message->header.type == MSG_TYPE_ACTUATOR_CMD) {
        const ActuatorPayload *actuator =
            reinterpret_cast<const ActuatorPayload *>(message->payload);
        if (actuator) {
    ...
    if (message->header.type == MSG_TYPE_SENSOR_DATA) {
        ...
        if (sensor) {
            logger_.info("Received sensor data: type=%d, len=%d", sensor->sensor_type,
                         sensor->data_length);
            return true;
        }
    }
    ...
    if (message->header.type == MSG_TYPE_UPDATE_AVAILABLE) {
        const UpdateAvailablePayload *update_payload =
        ...
        if (update_payload) {
            logger_.info("Received UPDATE_AVAILABLE: has_update=%d, type=%d, seq=%d",
                         update_payload->has_update, update_payload->update_type,
    ...
    if (message->header.type == MSG_TYPE_HEARTBEAT_RESPONSE) {
        const HeartbeatResponsePayload *heartbeat_response =
            reinterpret_cast<const HeartbeatResponsePayload *>(message->payload);
        if (heartbeat_response) {
            logger_.info("Received HEARTBEAT_RESPONSE: %04d-%02d-%02d %02d:%02d:%02d (dow=%d)",
                         heartbeat_response->year, heartbeat_response->month,
                         heartbeat_response->day, heartbeat_response->hour, heartbeat_response->min,
                         heartbeat_response->sec, heartbeat_response->dotw);
    ...
    if (!is_transmitting_ && pending_messages_.empty() && !message_queue_.empty()) {
        ...
        if (sendMessage(outgoing.buffer.get(), outgoing.length)) {
            logger_.info("Sent %s message (seq=%d) to 0x%04X from queue (attempt %d)",
                         RetryPolicy::getPolicyName(outgoing.criticality), outgoing.seq_num,
                         msg->header.dst_addr, outgoing.attempts);
...
bool ReliableMessenger::sendMessage(const uint8_t *buffer, size_t length)
{
    ...
    if (length >= 12) {
        ...
        if (msg->header.type == MSG_TYPE_ACTUATOR_CMD && msg->payload[1] == 0) {
            logger_.info("Sending OFF cmd to radio: len=%zu, payload[1]=%d", length,
                         msg->payload[1]);
        }
    }
...
void ReliableMessenger::handleAck(const AckPayload *ack_payload)
{
    ...
    if (it != pending_messages_.end()) {
        ...
        logger_.info("ACK received for seq=%d, status=%d", ack_payload->ack_seq_num,
                     ack_payload->status);
        ...
        if (network_stats_) {
            network_stats_->recordMessageSent(pending.dst_addr, pending.criticality, true,
                                              pending.attempts);
...
void ReliableMessenger::sendAck(uint16_t src_addr, uint8_t seq_num, uint8_t status)
{
    ...
    size_t length =
        MessageBuilder::createMessage(MSG_TYPE_ACK, 0, node_addr_, src_addr, my_seq, ack, buffer);
...
bool ReliableMessenger::wasRecentlySeen(uint16_t src_addr, uint8_t seq_num)
{
    ...
    for (size_t i = 0; i < SEEN_MESSAGE_BUFFER_SIZE; i++) {
        ...
        if (seen.src_addr == src_addr && seen.seq_num == seq_num && seen.timestamp != 0 &&
